...
/**
 * 是否实例化bean 指标
 *  1. 判断SnowFlakeEnableCondition类是否为true
 *  2. 根据返回判断是否实例化该bean
 */
@Conditional(SnowFlakeEnableCondition.class)
public class SnowFlake implements InitializingBean {
    ...
    public class HoldMachineIdTask implements Runnable {
        @Override
        public void run() {
            String key = "snowFlakeMachineId" + machineId;
...
//        }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        log.info("===========holdMachineIdTask 任务启动完成=========间隔=" + (3 * 60 * 1000));
    }


    private Long initMachineId() {
        RLock lock = redissonClient.getLock("snowFlakeMachineIdLock");//加锁
        ...
        try {
            ...
            do {
                ...
                if (Objects.nonNull(value) && value > 0) {
                    log.info("machineId:{}已被占用", machineId);
                    continue;
                }
                result = keyObject.trySet(machineId, 10, TimeUnit.MINUTES);//设置10分钟

            } while (!result);
            log.info("本实例获取到machineId:{}", machineId);
